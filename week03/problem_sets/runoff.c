...
#define MAX_VOTERS 5  //100
#define MAX_CANDIDATES 3  //9
...
bool vote(int voter, int rank, string name)
{
    // Check if the vote matches with candidates
    for (int i = 0; i < candidate_count; i++)
    {
...
void tabulate(void)
{
    // Assign +1 vote to all non eliminated candidates in the first rank
    for (int i = 0; i < voter_count; i++)
    {
...
int find_min(void)
{
    ...
    // Sort all candidates votes in crescent order
    for (int i = 0; i < candidate_count; i++)
    {
        if (!candidates[i].eliminated)
        {
